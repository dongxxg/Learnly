...
以下技术适用于调试类任务，与5步方法论配合使用。

### 稳定复现

不能稳定复现的 bug 无法验证修复。找到精确复现步骤；若无法复现，继续收集数据，不要猜。

### 检查最近变更

`git diff`、新依赖、配置变更、环境差异——bug 常藏在这里。

### 多组件系统打桩

在每个组件边界记录输入输出，验证配置传递。跑一次，分析证据，定位故障组件。

### 模式分析（找对标）

1. **找可工作的同类代码** — 同一代码库中类似但正常工作的代码在哪
2. **对照参考实现** — 完整读完参考实现，不要跳读
3. **识别差异** — 列出工作和故障之间的所有差异，再小的点都别忽略
4. **理解依赖** — 这个组件依赖什么？配置？环境？有什么假设？

### 先写失败测试再修

最简复现用例，自动化或用脚本都行。必须失败才进下一步。若涉及异步等待，用条件轮询替代固定延迟（见 `${HARNESS_ROOT:-.claude}/skills/pua-debugging/references/condition-based-waiting.md`）。

### 纵深防御

修完 bug 后，在数据流经的每一层加校验（入口/业务逻辑/环境/调试），让同类 bug 在结构上不可能再发生。详见 `${HARNESS_ROOT:-.claude}/skills/pua-debugging/references/defense-in-depth.md`。

### 根因追溯

沿调用链反向追溯，找到最初触发点，在源头修复而非报错处。详见 `${HARNESS_ROOT:-.claude}/skills/pua-debugging/references/root-cause-tracing.md`。
...
与抗合理化表互补——这里针对调试方法论错误，而非借口。

| 反模式 | 现实 |
|--------|------|
| "改一下试试看" | 瞎猜浪费时间。先找根因。 |
| "修完再补测试" | 没测试的修复站不住。先写测试。 |
| "一次改多处更省时间" | 无法判断哪个改动生效。还会引入新 bug。 |
| "我看到问题了，直接修" | 看到症状 ≠ 理解根因。 |
| "再试最后一次"（已失败 2+ 次） | 3 次以上 = 架构问题。升级，不要继续改。 |
| "参考太长，我凭感觉改" | 一知半解必定出 bug。完整读完。 |
| "紧急情况，没时间走流程" | 系统化比瞎试更快。越急越要走流程。 |
| "小问题，不需要流程" | 小问题也有根因。流程对简单 bug 同样高效。 |
...
失败次数越多，压力升级。风味按需选用，叠加效果更佳。第二轮开始用 Read 工具加载 `${HARNESS_ROOT:-.claude}/skills/pua-debugging/references/pua-flavors.md` 获取完整风味原文。

### 🟠 阿里味（默认主味）
> 底层逻辑是什么？顶层设计在哪里？过程的抓手在哪？如何保证闭环？今天最好的表现，是明天最低的要求。

### 🟡 字节味（坦诚直接）
> Always Day 1。务实敢为，深入事实。Context, not control——上下文要自己去找，不是等人喂。

### 🔴 华为味（狼性奋斗）
> 烧不死的鸟是凤凰。胜则举杯相庆，败则拼死相救。让听得见炮声的人呼唤炮火。

### 🟢 腾讯味（赛马竞争）
> 赛马文化——赛不过就换一匹。我不听过程，只看结果。
...
### 🟣 拼多多味（绝对执行）
> 你不干，有的是人替你干。成功不是等来的，是拼出来的。

### 🟤 Netflix 味（Keeper Test）
> 如果你提出离职，我会奋力挽留你吗？我们是职业球队，不是家庭。

### ⬛ Musk 味（Hardcore）
> Extremely hardcore。只有 exceptional performance 才构成及格。

### ⬜ Jobs 味（A/B Player）
> A players 雇佣 A players。最好的不是好 30%，是好 50 倍。你有 Reality Distortion Field 吗？
...
- `simplify` — 修复完成后用 simplify 检查代码质量
...
| 文件 | 何时加载 | 作用 |
|------|---------|------|
| `${HARNESS_ROOT:-.claude}/skills/pua-debugging/references/root-cause-tracing.md` | 追踪数据流、沿调用栈找根源时 | 沿调用链反向追溯 bug 源头，含诊断探针和污染源定位 |
| `${HARNESS_ROOT:-.claude}/skills/pua-debugging/references/defense-in-depth.md` | 修复完成后加固时 | 四层校验（入口/业务/环境/调试），让 bug 结构上不可能再发生 |
| `${HARNESS_ROOT:-.claude}/skills/pua-debugging/references/condition-based-waiting.md` | 测试涉及异步等待时 | 条件轮询替代固定延迟，消灭不稳定测试 |
